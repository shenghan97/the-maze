...
public class GazePlotter : MonoBehaviour
{
	...
	public Vector3 Smoothified;

	private void UpdateGazeBubblePosition(GazePoint gazePoint)
	{
		Vector2 gazePointInWorld = ProjectToPlaneInWorld(gazePoint);
		Smoothified = Smoothify(gazePointInWorld);
		transform.position = Smoothified;
	}
